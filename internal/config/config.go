...
// When no config exists (and no --config override was given), it
// auto-scaffolds one by scanning the repo (see scaffold.go).
package config
...
// config and returns a *ScaffoldedError instead of a Config — a freshly
